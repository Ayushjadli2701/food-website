...
const Order = ({ data, className, filterFood }) => {
  return (
    <div className={`order-div ${className}`}>
      <div className='order-now'>
        <h1>ORDER NOW</h1>
      </div>

      <div className='order-buttons'>
        <button className='btn' onClick={()=>filterFood("all")}>ALL</button>
        <button className='btn' onClick={()=>filterFood("breakfast")}>BREAKFAST</button>
        <button className='btn' onClick={()=>filterFood("lunch")}>LUNCH</button>
